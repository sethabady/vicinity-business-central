...
page 50160 "VICBatchItemJournal"
{
    ...
    layout
    {
        area(content)
        {
            ...
            repeater(Control1)
            {
                ...
                field("Batch Number"; Rec."Vicinity Batch No.")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'Specifies the Vicinity batch number associated with the item on the journal line.';
                    Caption = 'Vicinity Batch No.';
                    Visible = true;
                }

                field("VIC Batch Source Type"; Rec.VICBatchItemJournalSourceType)
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Vicinity Source';
                    Visible = false;
                }

                field(VICTransactionEntityType; VICTransactionEntityType)
                {
                    ApplicationArea = Basic, Suite;
                    Caption = 'Vicinity Batch Source';
                    ToolTip = 'Specifies the batch source of the journal line.';
                    Visible = true;

                    trigger OnValidate()
                    begin
                        Rec.VICBatchItemJournalSourceType := VICTransactionEntityType;
                        Rec.Validate(VICBatchItemJournalSourceType);
                    end;
                }
                ...
                field("VIC Quantity Remaining";Rec."VIC Quantity Remaining")
                {
                    ApplicationArea = Basic, Suite;
                    ToolTip = 'The Vicinity batch quantity remaining to be consumed or completed.';
                    Caption = 'Vicinity Batch Quantity Remaining';
                    Visible = true;
                }
    ...
    actions
    {
        ...
        area(processing)
        {
            group("F&unctions")
            {
                Caption = 'F&unctions';
                Image = "Action";
                action("&Load From Vicinity Batch")
                {
                    ApplicationArea = Basic, Suite;
                    Caption = '&Load From Vicinity Batch';
                    Ellipsis = true;
                    Image = CalculateConsumption;
                    ToolTip = 'Load journal lines from a Vicinity Batch.';

                    trigger OnAction()
                    var
                    begin
                        GetVicinityBatch.SetItemJnlLine(Rec);
                        GetVicinityBatch.InitializeItemJournalLine(Rec);
                        GetVicinityBatch.RunModal();
                        Clear(GetVicinityBatch);
                        CurrPage.Update(false);
                    end;
                }
                action("E&xplode BOM")
        ...
        area(Promoted)
        {
            group(Category_Process)
            {
                ...
                actionref("&Load From Vicinity Batch_Promoted"; "&Load From Vicinity Batch")
                {
                }
    ...
    begin
        ...
        // Vicinity
        VICTransactionEntityType := Rec.VICBatchItemJournalSourceType;
    ...
    begin
        ...
        EntryType := Rec."Entry Type";
        Rec.VICBatchItemJournalSourceType := Rec.VICBatchItemJournalSourceType::EndItem;
        VICTransactionEntityType := Rec.VICBatchItemJournalSourceType;
    ...
    begin
        ...
        VicinityIsConfigured := false;
        if VicinitySetup.Get()
        then begin
            VicinityIsConfigured := StrLen(VicinitySetup.ApiUrl) > 0;
            if VicinityIsConfigured then begin
    ...
    var
        GetVicinityBatch: Report "VICGetVicinityBatch";
        VICTransactionEntityType: Enum VICBatchItemJournalSourceType;
